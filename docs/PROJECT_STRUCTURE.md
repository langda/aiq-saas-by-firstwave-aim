...
│   └── proxy.ts                # session refresh + coarse auth gating (Next 16
...
├── globals.css                 # design tokens (light/dark) — shadcn convention
...
- **Promotion rule**: a component starts in its feature. When a _second_ feature needs it, it moves to `shared/` — never copy-paste between features (PROJECT_RULES: no duplication), and never build in `shared/` speculatively.
...
- **Query placement**: only in `features/*/server/db.ts`. A query used by two features lives in the feature that _owns_ the table (results queries live in `results`, even if `certificates` needs one — it goes through the results service).
